...
Understanding how, and if, sources of forecast uncertainty systematically covary with ecological characteristics will allow us to delimit the potential limits to forecasts based on our deep understanding of ecological systems.
...
Specifically, we pose three hypotheses motivated by recent reviews of ecological forecasting [@Petchey2015; @Houlahan2017], which show that understanding the limits to ecological forecasts requries understanding the effects of ecological processes and variables on forecast uncertainty.
...
\end{align}
\vspace{-2em}
where $y_t$ is the observed state at time *t*, $z_t$ is the latent state at time *t*, $\mu_t$ is the determinstic prediction of the state at time *t* given estimated parameters ($\boldsymbol{\theta_p}$) for the specified model function [*f()*], the latent state *z* at time t-1, and **x** is a vector climate covariates.
...
\item[January 2019] \textit{Second meeting for four days at Powell Center}: Analysis teams present results for feedback from the working group. Finalize database and identify any outstanding issues. Evaluate manuscript drafts and form writing teams. Create a \textsc{GitHub} repository for individual forecasts.
...
The assembled database of plant and animal abundance time series, coupled with environmental covariates, will be an invaluable resource for teaching and research, and the repository of population forecasts will provide new opportunities to validate, and improve, forecasts.
