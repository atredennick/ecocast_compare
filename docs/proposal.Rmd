...
\new{For example, the Earth's weather is a chaotic system, meaning its dynamics are internally unstable and sensitive to initial conditions uncertainty.
...
\new{In contrast, ecologists are attempting to make actionable forecasts with little knowledge of which term in Eq. 2 dominates forecast error.
...
While it is impossible to partition forecast uncertainty for every species on Earth, links among the dominate source of forecast error and species' life histories might serve as prior information that can guide new monitoring and modeling efforts.}
...
\new{Doing so will (1) provide actionable information on new ways to improve ongoing ecological forecasts, (2) create a bridge between ecological theory and ecological forecasts, and (3) quantify the current limits to ecological forecasts.}
...
where $y_t$ is the observed state at time *t*, $z_t$ is the latent state at time *t*, $\mu_t$ is the determinstic prediction of the state at time *t* given estimated parameters ($\boldsymbol{\theta}$) for the specified model function [*f()*], the latent state *z* at time *t*-1, and **x** is a vector of environmental covariates.
...
\new{Using different distributions for the data and process models also helps make variance parameters identifiable when replicate observations are not available} [@Hobbs2015], \new{thus avoiding a potential pitfall of state-space models} [@Auger-Methe2016].
...
\new{The key result is that parameter uncertainty dominates forecast uncertainty at all horizons, with a shift toward driver and process error atr later horizons (Fig. 1B).
...
\new{Several participants bring expert knowledge of a particular dataset, some bring analytical experitise, and many bring both.
...
At the second meeting, analysis teams will present results, at which point the entire working group will work together to trouble-shoot lingering problems.}
...
\vspace{-2em}
*Confirmed participant;  $^\dagger$Local (Ft. Collins) participant; $^1$Technical liaison to Powell Center computing staf; $^2$Party responsible for adherence to Powell Center Data and Information Policy
...
\item[January to March 2019] Monthly PI Skype meetings to prepare for first meeting. Begin email exchanges to confirm participants. Write \texttt{R} scripts to download, clean, and aggregate data.
\item[April 2019] \textit{First meeting for four days at Powell Center}: Identify other appropriate data sets. Decide on model structures for each data set and identify climate data availability. Form two-person analysis teams for each data set. Define manuscripts.
\item[May 2019 to March 2020] Bi-monthly PI Skype meetings. Continue and finalize data set aquisition and aggregate into single database for analysis teams. Analysis teams, with the PC Fellow, identify appropriate climate drivers for their system and begin fitting state-space models. PC Fellow writes generalizable \texttt{R} functions for analysis teams. PC Fellow Skypes with each analysis team as needed to complete analyses. Write first drafts of manuscripts.
\item[April 2020] \textit{Second meeting for four days at Powell Center}: Analysis teams present results for feedback. Finalize database and identify any outstanding issues. Evaluate manuscript drafts and form writing teams. Create a GitHub repository for forecasts.
\item[May to September 2020] Finalize database and forecast repository. Finalize all forecasts and forecast partitions. Continue writing manuscripts. PC Fellow will have at least one Skype meeting with each analysis team during this period.
\item[September to December 2020] Complete manuscripts and submit for publication. Release database and forecast repository to Powell Center for permanent archiving.

\end{description}

\section{Anticipated Results and Benefits}
\new{Ecological forecasts are central to ecosystem mangament, either explicitly or implicitly.
